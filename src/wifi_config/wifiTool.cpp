...
boolean WifiTool::connectAttempt(String ssid, String password)
{
  ...
  // IPAddress local_IP(172, 24, 1, 75);
...
void WifiTool::setUpAPService()
{
  ...
  WiFi.mode(WIFI_AP);
  WiFi.softAPConfig(IPAddress(172, 24, 1, 75), IPAddress(172, 24, 1, 1), IPAddress(255, 255, 255, 0));
  WiFi.softAP("ESP_CONFIG");
  delay(500);

  /* Setup the DNS server redirecting all the domains to the apIP */
  dnsServer->setErrorReplyCode(DNSReplyCode::NoError);
  dnsServer->start(DNS_PORT, "*", IPAddress(172, 24, 1, 75));
